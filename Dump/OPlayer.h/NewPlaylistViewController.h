//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIViewController.h"

@class UIButton, UIImageView, UILabel, UITextField, UIView;

__attribute__((visibility("hidden")))
@interface NewPlaylistViewController : UIViewController
{
    UILabel *errMsg;
    UITextField *playlistName;
    BOOL isNew;
    int playlistIndexForRename;
    UIButton *buttonSave;
    UIImageView *imageTopView;
    UIView *topView;
    UIImageView *backImageView;
    UIImageView *backImageViewShadow;
    UILabel *titleLabel;
}

@property(retain, nonatomic) UILabel *titleLabel; // @synthesize titleLabel;
@property(retain, nonatomic) UIImageView *backImageViewShadow; // @synthesize backImageViewShadow;
@property(retain, nonatomic) UIImageView *backImageView; // @synthesize backImageView;
@property(retain, nonatomic) UIView *topView; // @synthesize topView;
@property(retain, nonatomic) UIImageView *imageTopView; // @synthesize imageTopView;
@property(retain, nonatomic) UIButton *buttonSave; // @synthesize buttonSave;
@property int playlistIndexForRename; // @synthesize playlistIndexForRename;
@property BOOL isNew; // @synthesize isNew;
@property(retain, nonatomic) UITextField *playlistName; // @synthesize playlistName;
@property(retain, nonatomic) UILabel *errMsg; // @synthesize errMsg;
- (void)textFieldDone:(id)arg1;
- (void)textFieldEditing:(id)arg1;
- (void)savePlaylist:(id)arg1;
- (void)cancelView:(id)arg1;
- (void)dealloc;
- (void)viewDidUnload;
- (void)didReceiveMemoryWarning;
- (void)relocateSubViews:(int)arg1;
- (void)didRotateFromInterfaceOrientation:(int)arg1;
- (void)willRotateToInterfaceOrientation:(int)arg1 duration:(double)arg2;
- (BOOL)shouldAutorotateToInterfaceOrientation:(int)arg1;
- (unsigned int)supportedInterfaceOrientations;
- (BOOL)shouldAutorotate;
- (void)viewWillAppear:(BOOL)arg1;
- (void)viewDidLoad;

@end

