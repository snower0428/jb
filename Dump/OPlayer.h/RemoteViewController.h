//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIViewController.h"

#import "UIActionSheetDelegate.h"
#import "UISearchBarDelegate.h"
#import "UITableViewDataSource.h"
#import "UITableViewDelegate.h"
#import "UITextFieldDelegate.h"

@class NSMutableArray, NSString, UIButton, UIImageView, UILabel, UISearchBar, UITableView, UITextField, UIView;

__attribute__((visibility("hidden")))
@interface RemoteViewController : UIViewController <UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource, UIActionSheetDelegate, UITextFieldDelegate>
{
    UITableView *myTableView;
    UISearchBar *mySearchBar;
    NSMutableArray *tableData;
    NSMutableArray *bookmarksArray;
    UITextField *textFieldUrlInput;
    UIButton *CustomClearAllButton;
    UIButton *CustomBackButton;
    UIButton *CustomEditButton;
    UIButton *playingButton;
    BOOL isEditMode;
    UIImageView *imageTopView;
    UIView *topView;
    UIImageView *backImageView;
    UIImageView *backImageViewShadow;
    UILabel *titleLabel;
    UIImageView *imageAddressView;
}

@property(retain, nonatomic) UIImageView *imageAddressView; // @synthesize imageAddressView;
@property(retain, nonatomic) UILabel *titleLabel; // @synthesize titleLabel;
@property(retain, nonatomic) UIImageView *backImageViewShadow; // @synthesize backImageViewShadow;
@property(retain, nonatomic) UIImageView *backImageView; // @synthesize backImageView;
@property(retain, nonatomic) UIView *topView; // @synthesize topView;
@property(retain, nonatomic) UIImageView *imageTopView; // @synthesize imageTopView;
@property(retain, nonatomic) UIButton *playingButton; // @synthesize playingButton;
@property(retain, nonatomic) UIButton *CustomEditButton; // @synthesize CustomEditButton;
@property(retain, nonatomic) UIButton *CustomBackButton; // @synthesize CustomBackButton;
@property(retain, nonatomic) UIButton *CustomClearAllButton; // @synthesize CustomClearAllButton;
@property(retain, nonatomic) UITextField *textFieldUrlInput; // @synthesize textFieldUrlInput;
@property(retain, nonatomic) NSMutableArray *bookmarksArray; // @synthesize bookmarksArray;
@property(retain, nonatomic) NSMutableArray *tableData; // @synthesize tableData;
@property(retain, nonatomic) UISearchBar *mySearchBar; // @synthesize mySearchBar;
@property(retain, nonatomic) UITableView *myTableView; // @synthesize myTableView;
- (void)currentPlaying:(id)arg1;
- (void)showPlayingOrEdit;
- (void)showEditButton;
- (void)showPlayingButton;
- (void)playViewControllerCloseNotify;
- (void)playInBackgroundNotify;
- (BOOL)textFieldShouldReturn:(id)arg1;
- (void)actionSheet:(id)arg1 didDismissWithButtonIndex:(int)arg2;
- (void)clearHistory:(id)arg1;
- (void)tableView:(id)arg1 commitEditingStyle:(int)arg2 forRowAtIndexPath:(id)arg3;
- (void)searchBarSearchButtonClicked:(id)arg1;
- (void)searchBarCancelButtonClicked:(id)arg1;
- (void)searchBar:(id)arg1 textDidChange:(id)arg2;
- (void)textChanged:(id)arg1;
- (void)searchBarTextDidEndEditing:(id)arg1;
- (void)searchBarTextDidBeginEditing:(id)arg1;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (int)tableView:(id)arg1 numberOfRowsInSection:(int)arg2;
- (int)numberOfSectionsInTableView:(id)arg1;
- (void)dealloc;
- (void)viewDidUnload;
- (void)didReceiveMemoryWarning;
- (void)relocateSubViews:(int)arg1;
- (void)didRotateFromInterfaceOrientation:(int)arg1;
- (void)willRotateToInterfaceOrientation:(int)arg1 duration:(double)arg2;
- (BOOL)shouldAutorotateToInterfaceOrientation:(int)arg1;
- (unsigned int)supportedInterfaceOrientations;
- (BOOL)shouldAutorotate;
- (void)gotoURL:(id)arg1;
- (void)customBack:(id)arg1;
- (void)CustomEdit:(id)arg1;
- (void)viewWillAppear:(BOOL)arg1;
- (void)viewDidLoad;
- (void)myMovieFinishedCallback:(id)arg1;
- (void)playMovieAtURL:(id)arg1;
- (id)settingsFilePath;
- (id)bookmarksFilePath;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;

@end

